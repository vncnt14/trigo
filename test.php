...
if (mysqli_num_rows($result) > 0) {
    while ($booking = mysqli_fetch_assoc($result)) {
        ...
        <p> You can Confirmed the booking</p>
            ...
            <strong>Number of passenger:</strong> <?php echo $booking['numpass']; ?><br>
             ...
             <!-- Dropdown for Driver's Time Arrival -->
    <label for="driverArrivalMinutes" class="block mt-2 text-lg">Driver's Time Arrival (minutes):</label>
    <select id="driverArrivalMinutes" name="driverArrivalMinutes" class="text-lg mb-2">
        <!-- Add options for minutes (you can customize the range as needed) -->
        <?php for ($i = 0; $i <= 59; $i++) : ?>
    ...
    <button onclick="confirmBooking()" class="button bg-green-500 text-white px-4 py-2 inline-block">Confirm Booking</button>
</div>
...
<script>
     function confirmBooking() {
        // Retrieve the selected minutes from the dropdown
        var selectedMinutes = document.getElementById('driverArrivalMinutes').value;

        // Do something with the selected minutes, such as submitting a form or making an AJAX request
        alert("Driver's Time Arrival (minutes): " + selectedMinutes);
    }
    function showAlert() {
